...
export default function ProtectedRoute({ allow, redirectTo = '/auth', children }) {
  ...
  const company = (() => { try { return useCompany(); } catch { return null; } })();
  ...
  const isAllowed = Array.isArray(allow) && allow.length > 0 ? allow.some((r) => roleToState[r]?.isAuthenticated) : (sa?.isAuthenticated || broker?.isAuthenticated || company?.isAuthenticated || appUser?.isAuthenticated);

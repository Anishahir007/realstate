...
const STORAGE_KEY = 'realestate_company_auth';
...
export function CompanyProvider({ children }) {
  ...
  useEffect(() => {
    async function fetchProfile(activeToken) {
      try {
        ...
        if (data && data.data && data.data.role === 'company') {
          const p = data.data;
          setState(prev => ({
            ...(prev || {}),
            token: activeToken,
  ...
  async function login(email, password) {
    let data;
    try {
      ({ data } = await axios.post(`${apiBase}/api/auth/company/login`, { email, password }, {
    ...
    try {
      ...
      if (data2 && data2.data && data2.data.role === 'company') {
        const p = data2.data;
        const updated = {
          token,
          profile: p,
  ...
  async function updateProfile(payload) {
    ...
    const { data } = await axios.put(`${apiBase}/api/auth/company/profile`, formData, {
      headers: {
...
export function useCompany() {
  ...
  if (ctx === undefined) throw new Error('useCompany must be used within CompanyProvider');
  return ctx;
}

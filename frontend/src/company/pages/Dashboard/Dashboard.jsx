...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function loadStats() {
      if (!company?.token || !company?.apiBase) return;
      try {
        ...
        const [statsRes] = await Promise.all([
          axios.get(`${company.apiBase}/api/company/dashboard-stats${statsQuery}`, {
  ...
  }, [company?.token, company?.apiBase, filter.key, filter.from, filter.to, filter.monthValue, filter.yearValue]);
  ...
  useEffect(() => {
    ...
    async function loadTrends() {
      ...
      try {
        ...
        const { data } = await axios.get(`${company.apiBase}/api/properties/listproperty?${params.toString()}`, {
          headers: { Authorization: `Bearer ${company.token}` },
        });
        if (!cancelled) {
    ...
    loadTrends();
    return () => { cancelled = true; };
  }, [company?.token, company?.apiBase, trendYear, filter.key, filter.from, filter.to, filter.monthValue, filter.yearValue]);
  ...
  return (
    <div className="companydashboard-root">
      <header className="companydashboard-header">
        <div>
          <h1 className="companydashboard-title">Dashboard</h1>
          <div className="companydashboard-sub">Overview of your properties and leads</div>
        </div>
        <div className="companydashboard-filterwrap">
          <button
            type="button"
            ref={filterButtonRef}
            className="companydashboard-filterbtn"
            onClick={() => setFilterMenuOpen((open) => !open)}
          >
            {filter.label}
            <span className="companydashboard-caret">▾</span>
          </button>
          {filterMenuOpen && (
            <div className="companydashboard-filtermenu" ref={filterMenuRef}>
              <div className="companydashboard-filtermenu-section">
                <div className="companydashboard-filtermenu-label">Quick ranges</div>
                {QUICK_RANGE_OPTIONS.map((opt) => (
                  <button
                    key={opt.key}
                    type="button"
                    className={`companydashboard-filtermenu-item ${filter.key === opt.key ? 'active' : ''}`}
              ...
              <div className="companydashboard-filtermenu-section">
                <div className="companydashboard-filtermenu-label">Month</div>
                <div className="companydashboard-filtermenu-inputs">
                  <select value={monthParts.monthIndex} onChange={handleSelectMonth} className="companydashboard-select">
                    {MONTH_NAMES.map((name, idx) => (
                      <option key={idx} value={idx}>{name}</option>
                    ))}
                  </select>
                  <select value={monthParts.year} onChange={handleSelectMonthYear} className="companydashboard-select">
                  ...
                  <button
                    type="button"
                    className="companydashboard-filtermenu-apply"
              ...
              <div className="companydashboard-filtermenu-section">
                <div className="companydashboard-filtermenu-label">Year</div>
                <div className="companydashboard-filtermenu-inputs">
                  <input
                    type="text"
                    value={yearDraft}
                    ...
                    placeholder="YYYY"
                    maxLength={4}
                    className="companydashboard-input"
                  />
                  <button
                    type="button"
                    className="companydashboard-filtermenu-apply"
              ...
              <div className="companydashboard-filtermenu-section">
                <div className="companydashboard-filtermenu-label">Custom range</div>
                <div className="companydashboard-filtermenu-inputs">
                  <input
                    type="date"
                ...
                {customRangeError && (
                  <div className="companydashboard-filtermenu-error">{customRangeError}</div>
                )}
              </div>
            </div>
          )}
      ...
      <main className="companydashboard-main">
        <section className="companydashboard-cards">
          <div className="companydashboard-card">
            <div className="companydashboard-card-head">
              <div className="companydashboard-icon companydashboard-icon-blue">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" />
                  <polyline points="9 22 9 12 15 12 15 22" />
                </svg>
              </div>
              <div className="companydashboard-card-title">Total Properties</div>
            </div>
            <div className="companydashboard-metric">{formatNumber(stats.totalProperties)}</div>
            <div className="companydashboard-delta companydashboard-delta-up">All properties</div>
          </div>

          <div className="companydashboard-card">
            <div className="companydashboard-card-head">
              <div className="companydashboard-icon companydashboard-icon-green">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" />
                  <polyline points="22 4 12 14.01 9 11.01" />
                </svg>
              </div>
              <div className="companydashboard-card-title">Published</div>
            </div>
            <div className="companydashboard-metric">{formatNumber(stats.publishedProperties)}</div>
            <div className="companydashboard-delta companydashboard-delta-up">Active listings</div>
          </div>

          <div className="companydashboard-card">
            <div className="companydashboard-card-head">
              <div className="companydashboard-icon companydashboard-icon-purple">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <div className="companydashboard-card-title">Total Users</div>
            </div>
            <div className="companydashboard-metric">{formatNumber(stats.totalUsers)}</div>
            <div className="companydashboard-delta companydashboard-delta-up">Registered users</div>
          </div>

          <div className="companydashboard-card">
            <div className="companydashboard-card-head">
              <div className="companydashboard-icon companydashboard-icon-pink">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
                </svg>
              </div>
              <div className="companydashboard-card-title">Active Leads</div>
            </div>
            <div className="companydashboard-metric">{formatNumber(stats.activeLeads)}</div>
            <div className="companydashboard-delta companydashboard-delta-up">Updated in last 24h</div>
          </div>

          <div className="companydashboard-card">
            <div className="companydashboard-card-head">
              <div className="companydashboard-icon companydashboard-icon-teal">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <polyline points="22 12 18 12 15 21 9 3 6 12 2 12" />
                </svg>
              </div>
              <div className="companydashboard-card-title">New This Week</div>
            </div>
            <div className="companydashboard-metric">{formatNumber(stats.newThisWeek)}</div>
            <div className="companydashboard-delta companydashboard-delta-up">Properties added</div>
          </div>
        </section>

        <section className="companydashboard-grid">
          <div className="companydashboard-panel trend-card">
          ...
          <div className="companydashboard-panel">
            <div className="companydashboard-panel-head">
              <h2>Property Types</h2>
              <div className="companydashboard-sub">Distribution of available properties</div>
            </div>
            <div className="companydashboard-pie-chart" style={{ height: 300, position: 'relative' }}>
              {propertyTypesDistribution.length > 0 ? (
        ...
        <section className="companydashboard-panel companydashboard-properties">
          <div className="companydashboard-tableactions">
            <button
              type="button"
              ref={columnButtonRef}
              className="companydashboard-columns-btn"
            ...
            {columnMenuOpen && (
              <div className="companydashboard-colmenu" ref={columnMenuRef} role="menu">
                <div className="companydashboard-colmenu-head">Show columns</div>
                {COLUMN_OPTIONS.map((col) => (
                  <label key={col.id} className="companydashboard-colmenu-item">
                    <input
                      type="checkbox"
          ...
          <div className="companydashboard-tablewrap">
            <table className="companydashboard-table" role="table">
              <thead>
                <tr>
                  ...
                  {visibleColumns.map((col) => {
                    ...
                    return (
                      <th key={col.id} scope="col" className={`col-${col.id}`}>
                        <div className="companydashboard-th-filter">
                          <span>{col.label}</span>
                          {hasFilter && (
                            <div className="companydashboard-th-filter-btn-wrap">
                              <button
                                type="button"
                                data-filter-col={col.id}
                                className={`companydashboard-th-filter-btn ${currentFilter ? 'active' : ''}`}
                              ...
                              {isMenuOpen && (
                                <div
                                  className="companydashboard-filter-menu"
                                  ref={(el) => { if (el) filterMenuRefs.current[col.id] = el; }}
                                >
                                  <div className="companydashboard-filter-menu-item">
                                    <button
                                      type="button"
                                  ...
                                  {filterValuesForCol.map((val) => (
                                    <div key={val} className="companydashboard-filter-menu-item">
                                      <button
                                        type="button"
              ...
              <tbody>
                {propertiesLoading ? (
                  <tr>
                    <td colSpan={visibleColumns.length + 2} className="companydashboard-table-empty">Loading properties…</td>
                  </tr>
                  ...
                  paginatedFiltered.map((row) => {
                    ...
                    const imageSrc = row.image
                      ? (row.image.startsWith('http') ? row.image : `${company.apiBase}${row.image}`)
                  ...
                  <tr>
                    <td colSpan={visibleColumns.length + 2} className="companydashboard-table-empty">No properties found.</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          {hasMore && (
            <div style={{ textAlign: 'center', padding: '20px' }}>
              <button
                ...
                className="companydashboard-seemore-btn"
              >
                See More
              </button>
            </div>

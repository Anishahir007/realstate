...
const CompanyPanelHeader = () => {
  ...
  const handleSearchSubmit = (e) => {
    ...
    if (trimmedQuery.length >= 2) {
      navigate(`/company/search?q=${encodeURIComponent(trimmedQuery)}`);
      setSearchQuery('');
      setShowSearchDropdown(false);
      setSearchResults([]);
    }
  ...
  const handleResultClick = (result) => {
    if (result.type === 'property') {
      navigate(`/company/properties?highlight=${result.id}`);
    } else if (result.type === 'lead') {
      navigate(`/company/crm/lead/${result.id}`);
    }
    setShowSearchDropdown(false);
    setSearchQuery('');
    setSearchResults([]);
          ...
          {isMenuOpen && (
            <div className="brokerpanelheader-menu" role="menu">
              <button
                ...
                onClick={() => {
                  setIsMenuOpen(false);
                  navigate('/company/settings/view-profile');
                }}
              >
                View Profile
              </button>
...
export default CompanyPanelHeader;

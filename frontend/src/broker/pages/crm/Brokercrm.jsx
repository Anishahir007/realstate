...
export default function BrokerPanelBrokercrm() {
  ...
  function openView(lead) {
    navigate(`/broker/crm/lead/${lead.id}`);
  }
  ...
  return (
    <div className="brokercrm-root">
      <div className="brokercrm-head">
        <div>
          <h2 className="brokercrm-title">CRM - Lead Management</h2>
          <p className="brokercrm-sub">Manage your leads and track activities</p>
        </div>
        <div className="brokercrm-actions">
          <button className="brokercrm-btn" onClick={exportCsv}>Export</button>
          <button className="brokercrm-btn primary" onClick={openAdd}>
            <span style={{ marginRight: '6px' }}>+</span>Add New Lead
          </button>
        </div>
      </div>

      <div className="brokercrm-toolbar">
        <div className="brokercrm-search">
        ...
        <select className="brokercrm-select" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
      ...
      {/* Metrics Cards */}
      <div className="brokercrm-metrics">
        <div className="brokercrm-metric-card">
          <div className="brokercrm-metric-icon" style={{ background: '#fff4e6' }}>
          ...
          <div className="brokercrm-metric-content">
            <div className="brokercrm-metric-label">Total Leads</div>
            <div className="brokercrm-metric-value">{metrics.total}</div>
            <div className="brokercrm-metric-sub">+{metrics.newThisWeek} new this week</div>
          </div>
        </div>
        
        <div className="brokercrm-metric-card">
          <div className="brokercrm-metric-icon" style={{ background: '#dcfce7' }}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" />
              <circle cx="9" cy="7" r="4" />
              <path d="M23 21v-2a4 4 0 0 0-3-3.87" />
          ...
          <div className="brokercrm-metric-content">
            <div className="brokercrm-metric-label">Qualified Leads</div>
            <div className="brokercrm-metric-value">{metrics.qualified}</div>
            <div className="brokercrm-metric-sub">
        ...
        <div className="brokercrm-metric-card">
          <div className="brokercrm-metric-icon" style={{ background: '#dbeafe' }}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
            </svg>
          </div>
          <div className="brokercrm-metric-content">
            <div className="brokercrm-metric-label">Avg. Lead Score</div>
            <div className="brokercrm-metric-value">{metrics.avgScore}</div>
            <div className="brokercrm-metric-sub">Quality indicator</div>
          </div>
        </div>
        
        <div className="brokercrm-metric-card">
          <div className="brokercrm-metric-icon" style={{ background: '#ede9fe' }}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <polyline points="22 12 18 12 15 21 9 3 6 12 2 12" />
            </svg>
          </div>
          <div className="brokercrm-metric-content">
            <div className="brokercrm-metric-label">Active Leads</div>
            <div className="brokercrm-metric-value">{metrics.active}</div>
            <div className="brokercrm-metric-sub">Leads from last 24 hours</div>
          </div>
        </div>
      </div>

      {/* Leads Section */}
      <section className="brokercrm-card">
        <div className="brokercrm-section-header">
          <div>
            <h3 className="brokercrm-sectiontitle">My Leads ({filteredLeads.length})</h3>
            <p className="brokercrm-subtle">Manage and update your leads</p>
          </div>
          <div className="brokercrm-tableactions">
            <button
              type="button"
              ref={columnButtonRef}
              className="brokercrm-columns-btn"
            ...
            {columnMenuOpen && (
              <div className="brokercrm-colmenu" ref={columnMenuRef} role="menu">
                <div className="brokercrm-colmenu-head">Show columns</div>
                {COLUMN_OPTIONS.map((col) => (
                  <label key={col.id} className="brokercrm-colmenu-item">
                    <input
                      type="checkbox"
        ...
        <div className="brokercrm-tablewrap">
          <table className="brokercrm-table">
            <thead>
              <tr>
            ...
            <tbody>
              {loading && (
                <tr>
                  <td colSpan={visibleColumns.length} className="brokercrm-table-empty">Loading leads…</td>
                </tr>
              )}
              {!!error && !loading && (
                <tr>
                  <td colSpan={visibleColumns.length} className="brokercrm-error">{error}</td>
                </tr>
              )}
              {!loading && !error && filteredLeads.length === 0 && (
                <tr>
                  <td colSpan={visibleColumns.length} className="brokercrm-table-empty">No leads</td>
                </tr>
              )}
                  ...
                  {visibleColumns.map((col) => {
                    switch (col.id) {
                      case 'lead':
                        return (
                          <td key={col.id}>
                            <div className="brokercrm-leadcell">
                              <div className="brokercrm-avatar">{initials(l.full_name)}</div>
                              <div>
                                <div 
                                  className="brokercrm-textbold" 
                                  style={{ cursor: 'pointer', color: '#2563eb' }}
                                  onClick={() => openView(l)}
                                >
                                  {l.full_name}
                                </div>
                                <div className="brokercrm-textmuted">{l.city || '-'}</div>
                              </div>
                            </div>
                          </td>
                        );
                      case 'contact':
                        return (
                          <td key={col.id}>
                            <div className="brokercrm-textbold">{l.email}</div>
                            <div className="brokercrm-textmuted">{l.phone || '-'}</div>
                          </td>
                        );
                      case 'status':
                        return (
                          <td key={col.id}>
                            <span className={`brokercrm-badge status-${(l.status || 'new').toLowerCase()}`}>
                              {labelize(l.status)}
                            </span>
                          </td>
                        );
                      ...
                      case 'actions':
                        return (
                          <td key={col.id}>
                            <div className="brokercrm-actions-col">
                              <button
                                className="brokercrm-link"
                                onClick={() => openView(l)}
                                title="View"
                                aria-label="View"
      ...
      {showAdd && (
        <div className="brokercrm-modal-overlay" onClick={() => setShowAdd(false)}>
          <div className="brokercrm-modal" onClick={(e) => e.stopPropagation()}>
            <div className="brokercrm-modal-header">
              <h3>New Lead</h3>
              <button className="brokercrm-iconbtn" onClick={() => setShowAdd(false)} aria-label="Close">×</button>
            </div>
            <form onSubmit={submitAdd} className="brokercrm-formgrid">
              ...
              <div className="brokercrm-modal-actions">
                <button type="button" className="brokercrm-btn-light" onClick={() => setShowAdd(false)}>Cancel</button>
                <button type="submit" className="brokercrm-btn-dark">Save</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showEdit && (
        <div className="brokercrm-modal-overlay" onClick={() => setShowEdit(false)}>
          <div className="brokercrm-modal" onClick={(e) => e.stopPropagation()}>
            <div className="brokercrm-modal-header">
              <h3>Edit Lead</h3>
              <button className="brokercrm-iconbtn" onClick={() => setShowEdit(false)} aria-label="Close">×</button>
            </div>
            <form onSubmit={submitEdit} className="brokercrm-formgrid">
